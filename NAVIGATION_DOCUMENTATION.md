...
## 📦 stay_wallet_screans Klasöründeki Ekranlar

Projede `stay_wallet_screans` klasöründe **35 ekran** tanımı bulunmaktadır. Bunların **23 tanesi** kodda implement edilmiş, **12 tanesi** henüz eksiktir.

### ✅ Mevcut Ekranlar (23/35)

1. ✅ Dashboard (`staywallet_dashboard`)
2. ✅ Wallet Screen
3. ✅ Currency Exchange (`currency_exchange`)
4. ✅ Smart Card Management (`smart_card_management`)
5. ✅ Spending Notification (`spending_notification`)
6. ✅ Travel Services (`travel_services_marketplace_1/2`)
7. ✅ Digital Key Details (`digital_key_details`)
8. ✅ Trip Itinerary (`trip_itinerary`)
9. ✅ Trip Budgeting (`trip_budgeting`)
10. ✅ Uber Ride Tracking (`uber_ride_tracking`)
11. ✅ Frequent Flyer Management (`frequent_flyer_management`)
12. ✅ E-Visa Connectivity (`e_visa_connectivity`)
13. ✅ Checkout Bill Review (`checkout_bill_review`)
14. ✅ Order Confirmation (`order_confirmation`)
15. ✅ Order Tracking (`order_tracking`)
16. ✅ Post Trip Expense Report (`post_trip_expense_report`)
17. ✅ Connect Account (`connect_booking.com_account`, `connect_sixt_account`, `connect_uber_account`)
18. ✅ Linked Success (`booking.com_linked_successfully`, `sixt_linked_successfully`, `uber_linked_successfully`)
19. ✅ Merchant Offers (`merchant_offers`)
20. ✅ Loyalty Rewards (`loyalty_rewards`)
21. ✅ Profile Screen
22. ✅ Identity Verification (`identity_verification`)
23. ✅ Passport Scan (`passport_scan`)

### ❌ Eksik Ekranlar (12/35)

Aşağıdaki ekranlar `stay_wallet_screans` klasöründe tanımlı ancak kodda henüz implement edilmemiştir:

1. ❌ **AI Voice Assistant** (`ai_voice_assistant`) - Sesli asistan ekranı
2. ❌ **Allianz Insurance Linked Successfully** (`allianz_insurance_linked_successfully`) - Sigorta bağlantı başarı ekranı
3. ❌ **Allianz Travel Insurance Selection** (`allianz_travel_insurance_selection`) - Sigorta seçim ekranı
4. ❌ **E-Visa Linked Successfully** (`e_visa_linked_successfully`) - E-Visa bağlantı başarı ekranı
5. ❌ **Integrated Travel Hub 1** (`integrated_travel_hub_1`) - Entegre seyahat merkezi ekranı
6. ❌ **Integrated Travel Hub 2** (`integrated_travel_hub_2`) - Entegre seyahat merkezi ekranı
7. ❌ **Ordering Room Service Voice** (`ordering_room_service_voice`) - Sesli oda servisi sipariş ekranı
8. ❌ **Secondary Cardholder App View** (`secondary_cardholder_app_view`) - İkincil kart sahibi görünümü
9. ❌ **Select Uber Ride** (`select_uber_ride`) - Uber yolculuk seçim ekranı
...
12. ❌ **Wallet Screen** - Dashboard'dan ayrı bir ekran olabilir

**Not**: `travel_services_marketplace_1` ve `travel_services_marketplace_2` mevcut `TravelServicesScreen` ile aynı olabilir veya farklı durumlarını temsil ediyor olabilir.

Detaylı karşılaştırma için `SCREEN_COMPARISON.md` dosyasına bakabilirsiniz.
